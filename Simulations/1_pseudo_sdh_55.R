...
for(j in 1:nsim){
  ...
  #### Step 1 - Load Data ####
  data <- read.csv(paste0("//restricted//projectnb//conner-thesis//lifetimerisk//simulation//data_sdh//data", l, "_", j, ".csv"), header=TRUE, sep=",")
  
  data$cause[data$times>1] <- 0
  data$times[data$times>1] <- 1
  
  # Shift data to be similar to FHS; lifetime risk at age 95
  data$entry <- 55 + (40*round(data$entry, 5))
  data$times <- 55 + (40*round(data$times, 5))
...
colnames(res_cll) <- c('beta0', 'beta0_SE', 'beta0_pval', 'beta1', 'beta1_SE', 'beta1_pval', 
                         ...
                         'ltr1', 'ltr1_SE', 'ltr1_CIL', 'ltr1_CIU',
                         'ltrdiff', 'ltrdiff_SE', 'ltrdiff_CIL', 'ltrdiff_CIU')
write.csv(res_logit, paste0("//restricted//projectnb//conner-thesis//lifetimerisk//simulation//pseudo//sdhv2//logitres", l, ".csv"), row.names=FALSE)
write.csv(res_cll, paste0("//restricted//projectnb//conner-thesis//lifetimerisk//simulation//pseudo//sdhv2//cllres", l, ".csv"), row.names=FALSE)
