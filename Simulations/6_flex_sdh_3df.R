...
for(k in nstart:nend){
  ...
  dat <- read.csv(paste0("//restricted//projectnb//conner-thesis//lifetimerisk//simulation//data_sdh//data", l, "_", k, ".csv"), header=TRUE, sep=",")

  dat$cause[dat$times>1] <- 0
  dat$times[dat$times>1] <- 1
  
  dat$entry <- 55 + (40*round(dat$entry, 5)) # Slight transform to be consistent w/ motivating example
  ...
  tryCatch({fp <- stpm2(Surv(Tstart-55, Tstop-55, ev1) ~ arm, weight=weight.ltrc, data=dat.ltrc, df=3, tvc=list(arm=3), link='PH')
...
write.csv(res, paste0("//restricted//projectnb//conner-thesis//lifetimerisk//simulation//flex//sdhv2_sub55_3df//res_", l, "_", nend, ".csv"), row.names=FALSE)
